...
command = (
    ...
    + c["config"]["KEY_NAME"]
    + '","InstanceProfile":"EMR_EC2_DefaultRole","SubnetId":"'
    + c["config"]["SUBNET_ID"]
    + '","EmrManagedSlaveSecurityGroup":"'
    + c["config"]["WORKER_SECURITY_GROUP"]
    + '","EmrManagedMasterSecurityGroup":"'
    + c["config"]["MASTER_SECURITY_GROUP"]
    + "\"}' --service-role EMR_DefaultRole --release-label emr-5.23.0 --log-uri '"
    + c["config"]["S3_BUCKET"]
    + "' --name '"
    + c["config"]["EMR_CLUSTER_NAME"]
...
cluster_id = json.loads(cluster_id_json)["ClusterId"]
...
command = (
    ...
    + ".pem "
    + PATH
    + "/install_hail_and_python36.sh hadoop@"
    + master_dns
    + ":/home/hadoop"
)
os.system(command)
...
key = paramiko.RSAKey.from_private_key_file(
    os.path.expanduser(c["config"]["PATH_TO_KEY"] + c["config"]["KEY_NAME"] + ".pem")
)
